...
import 'lib/services/audio_analysis/note_detector_service.dart';
import 'lib/services/audio_analysis/note_verification_service_impl.dart';
...
void main() async {
  ...
  final thresholds = [0.25, 0.30, 0.33, 0.35, 0.38, 0.40, 0.45, 0.50, 0.55, 0.60];
  ...
  for (var threshold in thresholds) {
    ...
    // 格式化輸出
    final midi = results[0] >= 0 ? '${results[0].toStringAsFixed(1)}%' : '  -  ';
    final phone = results[1] >= 0 ? '${results[1].toStringAsFixed(1)}%' : '  -  ';
    final pc = results[2] >= 0 ? '${results[2].toStringAsFixed(1)}%' : '  -  ';
    final noise1 = results[3] >= 0 ? '${results[3].toStringAsFixed(1)}%' : '  -  ';
    final noise2 = results[4] >= 0 ? '${results[4].toStringAsFixed(1)}%' : '  -  ';
    ...
    print('│ ${threshold.toStringAsFixed(2).padRight(7)} │ ${midi.padLeft(8)} │ ${phone.padLeft(8)} │ ${pc.padLeft(8)} │ ${noise1.padLeft(8)} │ ${noise2.padLeft(8)} │ $scoreStr$marker');
  }
