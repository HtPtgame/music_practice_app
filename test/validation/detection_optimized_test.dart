...
void main(List<String> args) async {
  ...
  final testRounds = {
    '1': {
      ...
      'testCases': [
        {
          'path': 'assets/test_voice/生日快樂(midi轉檔).wav',
          'name': '生日快樂(midi轉檔)',
          'type': 'correct',
          'expectedF1': '>0.95'
        },
        {
          'path': 'assets/test_voice/生日快樂(手機環境錄製).wav',
          'name': '生日快樂(手機環境)',
          'type': 'correct',
          'expectedF1': '>0.85'
        },
        {
          'path': 'assets/test_voice/生日快樂(電腦環境錄製).wav',
          'name': '生日快樂(電腦環境)',
          'type': 'correct',
          'expectedF1': '>0.85'
        },
        {
          'path': 'assets/test_voice/小星星(手機環境錄製).wav',
          'name': '小星星(手機環境)',
          'type': 'wrong-song',
          'expectedF1': '<0.20'
        },
        {
          'path': 'assets/test_voice/名偵探柯南(手機環境錄製).wav',
          'name': '名偵探柯南(手機環境)',
          'type': 'wrong-song',
          'expectedF1': '<0.20'
        },
        {
          'path': 'assets/test_voice/測試音檔(手機環境錄製).wav',
          'name': '測試音檔(手機環境)',
          'type': 'wrong-song',
          'expectedF1': '<0.20'
        },
        {
          'path': 'assets/test_voice/環境背景.wav',
          'name': '環境背景',
          'type': 'noise',
          'expectedF1': '<0.05'
        },
        {
          'path': 'assets/test_voice/環境背景2.wav',
          'name': '環境背景2',
          'type': 'noise',
          'expectedF1': '<0.05'
        },
    ...
    '2': {
      ...
      'testCases': [
        {
          'path': 'assets/test_voice/測試音檔(midi轉檔).wav',
          'name': '測試音檔(midi轉檔)',
          'type': 'correct',
          'expectedF1': '>0.95'
        },
        ...
        {
          'path': 'assets/test_voice/測試音檔(電腦環境錄製).wav',
          'name': '測試音檔(電腦環境)',
          'type': 'correct',
          'expectedF1': '>0.85'
        },
    ...
    '3': {
      ...
      'testCases': [
        {
          'path': 'assets/test_voice/小星星(midi轉檔).wav',
          'name': '小星星(midi轉檔)',
          'type': 'correct',
          'expectedF1': '>0.90'
        },
        ...
        {
          'path': 'assets/test_voice/小星星(電腦環境錄製).wav',
          'name': '小星星(電腦環境)',
          'type': 'correct',
          'expectedF1': '>0.80'
        },
    ...
    '4': {
      ...
      'testCases': [
        {
          'path': 'assets/test_voice/名偵探柯南(midi轉檔).wav',
          'name': '名偵探柯南(midi轉檔)',
          'type': 'correct',
          'expectedF1': '>0.85'
        },
        ...
        {
          'path': 'assets/test_voice/名偵探柯南(電腦環境錄製).wav',
          'name': '名偵探柯南(電腦環境)',
          'type': 'correct',
          'expectedF1': '>0.75'
        },
...
Future<void> runTestRound(Map<String, dynamic> round) async {
  ...
  // 表格標題
  print(
      '${_pad("測試案例", 25)} ${_pad("類型", 12)} ${_pad("F1分數", 10)} ${_pad("評級", 6)} ${_pad("結果", 10)}');
  print('─' * 70);
  ...
  for (final result in results) {
    ...
    if (status == 'SUCCESS') {
      ...
      print(
          '${_pad(name, 25)} ${_pad(type, 12)} ${_pad('${(f1 * 100).toStringAsFixed(1)}%', 10)} ${_pad(grade, 6)} ${_pad(passIcon, 10)}');
      ...
      print(
          '${_pad(name, 25)} ${_pad(type, 12)} ${_pad('N/A', 10)} ${_pad('N/A', 6)} ${_pad('❌ ERROR', 10)}');
      errorCount++;
    }
  }
  ...
  print(
      '   📊 通過率: ${(passCount / (passCount + failCount + errorCount) * 100).toStringAsFixed(1)}%');
  print('');

  // 分類分析
  final correctTests = results
      .where((r) => r['type'] == 'correct' && r['status'] == 'SUCCESS')
      .toList();
  final wrongTests = results
      .where((r) => r['type'] == 'wrong-song' && r['status'] == 'SUCCESS')
      .toList();
  final noiseTests = results
      .where((r) => r['type'] == 'noise' && r['status'] == 'SUCCESS')
      .toList();

  if (correctTests.isNotEmpty) {
    print('🎵 正確演奏測試:');
    final avgF1 = correctTests
            .map((r) => r['f1Score'] as double)
            .reduce((a, b) => a + b) /
        correctTests.length;
    print('   平均 F1: ${(avgF1 * 100).toStringAsFixed(1)}%');
    print(
        '   通過: ${correctTests.where((r) => r['passTest'] == true).length}/${correctTests.length}');
    print('');
  }
  ...
  if (wrongTests.isNotEmpty) {
    print('❌ 錯誤曲目測試:');
    final avgF1 =
        wrongTests.map((r) => r['f1Score'] as double).reduce((a, b) => a + b) /
            wrongTests.length;
    print('   平均 F1: ${(avgF1 * 100).toStringAsFixed(1)}% (期望 <20%)');
    print(
        '   通過: ${wrongTests.where((r) => r['passTest'] == true).length}/${wrongTests.length}');
    final wrongDetected =
        wrongTests.where((r) => r['isProbablyWrong'] == true).length;
  ...
  if (noiseTests.isNotEmpty) {
    print('🔇 雜訊測試:');
    final avgF1 =
        noiseTests.map((r) => r['f1Score'] as double).reduce((a, b) => a + b) /
            noiseTests.length;
    print('   平均 F1: ${(avgF1 * 100).toStringAsFixed(1)}% (期望 <5%)');
    print(
        '   通過: ${noiseTests.where((r) => r['passTest'] == true).length}/${noiseTests.length}');
    final avgFP = noiseTests
            .map((r) => r['falsePositives'] as int)
            .reduce((a, b) => a + b) /
        noiseTests.length;
