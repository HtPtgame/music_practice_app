...
class PracticeTimerService extends ChangeNotifier with WidgetsBindingObserver {
  ...
  Future<void> _loadSettings() async {
    try {
      ...
      _showNotification =
          prefs.getBool('timer_show_notification') ?? true; // 預設開啟通知
      _pauseThresholdSeconds = prefs.getInt('timer_pause_threshold') ??
          _defaultPauseThresholdSeconds;
    } catch (e) {
  ...
  Future<void> stop() async {
    ...
    // 延遲後取消所有通知（包括停止通知），確保用戶看到通知後自動消失
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.paused) {
      ...
      if (_isRunning && _backgroundTime != null) {
        ...
        // -1 表示不暫停
        if (_pauseThresholdSeconds > 0 &&
            backgroundDuration > _pauseThresholdSeconds) {
          // 超過閾值，扣除背景時間
          debugPrint(
              '⚠️ 背景時間 $backgroundDuration 秒超過閾值 $_pauseThresholdSeconds 秒，扣除該段時間');
          // 重新設定開始時間，使背景時間不被計入
          _startTime = now;
        } else {
  ...
  void reset() {
    _stopCountingTimer();
    _stopNotificationUpdates();
    _cancelNotification(); // 完全清除通知
    _isRunning = false;
    _startTime = null;
    _accumulatedSeconds = 0;
    _backgroundTime = null;
    _stopRequested = false; // 清除停止請求標誌
    _safeNotifyListeners();
