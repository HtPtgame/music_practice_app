...
void main() async {
  ...
  if (!await file.exists()) {
    ...
    print('2. 修改 lib/services/audio_analysis/test_midi_parser.dart 中的 midiPath');
    print('3. 重新運行此腳本');
    return;
  }
  ...
  try {
    ...
    if (timeline.events.isNotEmpty) {
      ...
      print('   第一個音符: ${firstNote.noteName} (${firstNote.startTime.toStringAsFixed(2)}s)');
      print('   最後一個音符: ${lastNote.noteName} (${lastNote.endTime.toStringAsFixed(2)}s)');
      
      // 音域分析
      final allNotes = timeline.events.map((e) => e.midiNote).toSet().toList()..sort();
      final lowest = timeline.events.map((e) => e.midiNote).reduce((a, b) => a < b ? a : b);
      final highest = timeline.events.map((e) => e.midiNote).reduce((a, b) => a > b ? a : b);
      
      print('   音域: ${NoteEvent(midiNote: lowest, startTime: 0, endTime: 0).noteName} - '
