...
class OnsetEvent {
  ...
  @override
  String toString() =>
      'Onset(${time.toStringAsFixed(3)}s, strength: ${strength.toStringAsFixed(3)})';
}
...
class SpectralFluxOnsetDetector {
  ...
  List<OnsetEvent> detectOnsets(Spectrogram spectrogram) {
    ...
    print(
        '   平均強度: ${onsets.isEmpty ? 0 : (onsets.map((o) => o.strength).reduce((a, b) => a + b) / onsets.length).toStringAsFixed(3)}');
  ...
  List<OnsetEvent> _detectPeaks(
    ...
    final minIntervalFrames =
        (minOnsetInterval / spectrogram.timeResolution).round();
