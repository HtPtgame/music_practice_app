...
class DynamicParameterService {
  ...
  DynamicParameters calculateParameters(MidiTimeline timeline,
      {double? averageBpm}) {
  ...
  DifficultyLevel _assessDifficulty({
    ...
    if (noteCount <= beginnerNoteThreshold) {
      ...
      score += 10 +
          ((noteCount - beginnerNoteThreshold) /
                  (intermediateNoteThreshold - beginnerNoteThreshold) *
                  10)
              .toInt();
    } else if (noteCount <= advancedNoteThreshold) {
      score += 20 +
          ((noteCount - intermediateNoteThreshold) /
                  (advancedNoteThreshold - intermediateNoteThreshold) *
                  10)
              .toInt();
    } else {
      score += 30 +
          min(((noteCount - advancedNoteThreshold) / 500 * 10).toInt(), 10);
    }
    ...
    if (averageSpeed <= beginnerSpeedThreshold) {
      ...
      score += 10 +
          ((averageSpeed - beginnerSpeedThreshold) /
                  (intermediateSpeedThreshold - beginnerSpeedThreshold) *
                  10)
              .toInt();
    } else if (averageSpeed <= advancedSpeedThreshold) {
      score += 20 +
          ((averageSpeed - intermediateSpeedThreshold) /
                  (advancedSpeedThreshold - intermediateSpeedThreshold) *
                  10)
              .toInt();
    } else {
      score += 30 +
          min(((averageSpeed - advancedSpeedThreshold) / 5 * 10).toInt(), 10);
    }
