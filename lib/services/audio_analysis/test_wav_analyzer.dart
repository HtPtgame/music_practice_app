...
void main() async {
  ...
  if (!await file.exists()) {
    ...
    print('3. 修改 lib/services/audio_analysis/test_wav_analyzer.dart 中的 wavPath');
    print('4. 重新運行此腳本');
    print('');
  ...
  try {
    ...
    print('   頻率解析度: ${spectrogram.frequencyResolution.toStringAsFixed(2)} Hz/bin');
    print('   時間解析度: ${(spectrogram.timeResolution * 1000).toStringAsFixed(2)} ms/幀');
    print('   總時長: ${(spectrogram.timeFrames * spectrogram.timeResolution).toStringAsFixed(2)} 秒');
    ...
    for (int frame = 0; frame < 5 && frame < spectrogram.timeFrames; frame++) {
      ...
      print('   時間 ${time.toStringAsFixed(2)}s: 峰值頻率 ${peakFreq.toStringAsFixed(1)} Hz, '
