  ...
  Future<void> play(String midiPath) async {
    ...
    try {
      ...
      if (filteredEvents.isEmpty) {
        debugPrint(
            '⚠️ OptimizedMidiPlayerService: No valid piano events found.');
        return;
      }
      ...
      if (firstNoteDelayMs > 500) {
        ...
        adjustedTempos = parser.tempoEvents.map((tempo) {
          return TempoChange(
            tick:
                (tempo.tick - firstNoteTick).clamp(0, double.infinity).toInt(),
  ...
  void _startPlaybackLoop() {
    ...
    _playbackLoop = Timer.periodic(
      ...
      while (_currentIndex < _events.length &&
          _batchedEvents.length < _maxBatchSize) {
  ...
  Future<void> playNote(int noteNumber, {int velocity = 64}) async {
    ...
    if (_soundfontId != null && noteNumber >= 21 && noteNumber <= 108) {
      try {
        ...
        debugPrint(
            '❌ OptimizedMidiPlayerService: Error playing piano note $noteNumber: $e');
      }
    }
  }
  ...
  Future<void> stopNote(int noteNumber) async {
    if (_soundfontId != null && noteNumber >= 21 && noteNumber <= 108) {
      try {
        ...
        debugPrint(
            '❌ OptimizedMidiPlayerService: Error stopping piano note $noteNumber: $e');
      }
    }
  }
  ...
  Future<void> stopAllNotes() async {
    if (_soundfontId != null) {
      try {
        ...
        debugPrint(
            '❌ OptimizedMidiPlayerService: Error stopping all piano notes: $e');
      }
    }
  }
}
